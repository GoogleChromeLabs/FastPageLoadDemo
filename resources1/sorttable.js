var europeandate=!1;var alternate_row_colors=!0;addEvent(window,"load",sortables_init);var SORT_COLUMN_INDEX;var thead=!1;function sortables_init(){if(!document.getElementsByTagName)return;tbls=document.getElementsByTagName("table");for(ti=0;ti<tbls.length;ti++){thisTbl=tbls[ti];if(((' '+thisTbl.className+' ').indexOf("sortable")!=-1)&&(thisTbl.id)){ts_makeSortable(thisTbl)}}}
function ts_makeSortable(t){if(t.rows&&t.rows.length>0){if(t.tHead&&t.tHead.rows.length>0){var firstRow=t.tHead.rows[t.tHead.rows.length-1];thead=!0}else{var firstRow=t.rows[0]}}
if(!firstRow)return;for(var i=0;i<firstRow.cells.length;i++){var cell=firstRow.cells[i];var txt=ts_getInnerText(cell);if(cell.className!="unsortable"&&cell.className.indexOf("unsortable")==-1){cell.innerHTML='<a href="#" class="reg title" onclick="ts_resortTable(this, '+i+');return false;">'+txt+'</a>'}}
...
newRows.sort(sortfn);for(i=0;i<newRows.length;i++){if(!newRows[i].className||(newRows[i].className&&(newRows[i].className.indexOf('sortbottom')==-1))){t.tBodies[0].appendChild(newRows[i])}}
...
dt=date.substr(7,4)+mt+date.substr(0,2);return dt}else if(date.length==10){if(europeandate==!1){dt=date.substr(6,4)+date.substr(0,2)+date.substr(3,2);return dt}else{dt=date.substr(6,4)+date.substr(3,2)+date.substr(0,2);return dt}}else if(date.length==8){yr=date.substr(6,2);if(parseInt(yr)<50){yr='20'+yr}else{yr='19'+yr}
if(europeandate==!0){dt=yr+date.substr(3,2)+date.substr(0,2);return dt}else{dt=yr+date.substr(0,2)+date.substr(3,2);return dt}}
return dt}
...
{if(elm.addEventListener){elm.addEventListener(evType,fn,useCapture);return!0}else if(elm.attachEvent){var r=elm.attachEvent("on"+evType,fn);return r}else{alert("Handler could not be removed")}}
